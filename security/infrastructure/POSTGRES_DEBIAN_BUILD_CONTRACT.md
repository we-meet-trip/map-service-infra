...
## Root integration update — 2026-09-07

Run34092684257 built PostGIS3.5.7 but failed the builder/final runtime inventory comparison. The bounded build log did not contain both original tuples, so an actual version mismatch and conditional `binary:Package` architecture formatting remain distinct possible causes. The recipe now records separate Package/Architecture/Version fields, installs the eight exact allowlisted builder tuples in the final stage with normal signed APT and `--no-remove`, and retains strict comparison plus bounded mismatch diagnostics. PG17 pins, locale/UID and ldd checks remain. This correction still needs an actual remote candidate build, strict scan and restore fixture.

The base image's exact gosu1.19 binary was independently read and signature checked; it uses Go1.24.6 and apt does not replace it. The parent now embeds the unchanged pinned `gosu_builder` stage from Dockerfile.gosu-security, builds unchanged gosu1.19 source/modules with Go1.26.8, and copies only the new static helper plus metadata. Original upstream signatures apply only to original bytes; new hashes/source/runtime tests are separate. PG uses `/usr/share/map-candidate/gosu/` for those receipts so the existing owned, unstarted-container evidence exporter captures them. No compiler, original vulnerable executable or real credentials enter the runtime image. Full image strict scan and PG fixture remain mandatory, including unfixed findings.
