...
def command(args, *, env=None, timeout=300, cwd=REPO):
...
def compose_command(*, admin=False, bundle=None, env_file=None):
...
def preflight(bundle, env_file, env):
        ...
        with tempfile.TemporaryDirectory(prefix="incoming-", dir=STATE) as incoming:
            ...
            try:
                ...
                command(["bash", "scripts/cloud-up.sh", "--test", "--registry", "--vision", "--edge", "--admin", "--monitoring"],
                        env={**env, "RELEASE_BUNDLE": str(new_bundle)}, timeout=2400)
                status("smoke")
                smoke()
                (history / "result.json").write_text(json.dumps({"status": "complete", "run_id": data["github_run_id"], "infra_sha": data["infra_sha"]}))
                (STATE / "current.json").write_text(json.dumps({"bundle": str(new_bundle), "run_id": data["github_run_id"]}))
