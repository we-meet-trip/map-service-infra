...
#
# 지도 식별자도 함께 싣는다. 이 값을 받아 쓰던 정적 페이지는 지웠고 앱은
# 자기 설정에서 읽으므로, 지금 이 두 칸을 읽는 곳은 없다. 게시된 파일을
# 직접 받아 가는 쪽이 생길 수 있어 자리만 남겨 둔다. 이 식별자는 브라우저가
# 어차피 밖으로 내보내는 값이고 방어는 발급처의 도메인 목록이 한다.
#
# 예비 식별자도 함께 싣는다. 앞의 것으로 먼저 붙고 인증이 막히면 뒤의 것으로
# 한 번 더 시도하는 순서다.
