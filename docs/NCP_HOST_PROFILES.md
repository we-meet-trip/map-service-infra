...
운영 DB는 `map_prod`로 고정한다. 원격 암호문과 transport receipt 확인 후에는 이번 작업이 직접 만든 UUID 디렉터리의 marker·닫힌 manifest·모든 파일 SHA와 정확한 목록을 대조하고 평문 임시 사본만 정리한다. 임의 파일·symlink·다른 작업·기존 데이터가 섞이면 삭제를 시작하지 않는다. 원격 사본은 append-only이며 로컬 검증 receipt는 종류별 최근96개(30분 주기에서2일)를 보존한다. 실패한 작업은 `/srv/map-prod/backups`에 보존하며4개가 쌓이면 추가 수집을 거절해 운영자 검토를 요구한다. PG는 관측 DB 크기8배+2GiB 여유를 먼저 요구하고 Redis/전송 helper도 기존 용량 제한을 적용한다. 원격 보존기간·실패 작업 해소와 용량/마지막 성공시각 경보의 실제 수신은 별도 운영 gate다. 실제 NCP에서 key pair 복구, 두 번 이상의 timer/S3 암복호화 왕복, 새로운 DB restore·애플리케이션/권한/키·RPO/RTO 검증은 여전히 별도 인수 검사다. 단위검사의 mock transport 성공을 실제 NCP 암복호화·전체 서비스 복원으로 기록하지 않는다.
