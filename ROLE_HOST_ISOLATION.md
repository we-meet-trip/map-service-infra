...
## Candidate management credential and process privileges (2026-09-07)

`USER_ADMIN_INTERNAL_TOKEN` is a distinct Admin→User credential. Co-host Compose forwards it only to Admin and User. Each central `ADMIN_TARGETS` entry must supply its own value, independent from its `INTERNAL_SERVICE_TOKEN`; the role validator rejects missing, blank and reused values. User rejects all internal admin requests when this credential is absent or equals its ordinary serving token. Admin stops its User management request before opening a connection when the selected credential is absent/reused. Health, central login and ordinary Hub calls remain separately available. Provision fresh random values (at least 32 characters) privately per environment and deploy matching User/Admin source and configuration together. Older images with the old shared-token behavior are not a safe standalone rollback; preserve a reviewed paired manifest and configuration.

User receives ordinary Hub timeout from `USER_HUB_TIMEOUT_SECONDS` (default 5 seconds) and directions timeout from `HUB_DIRECTIONS_TIMEOUT_SECONDS` (default 20 seconds). A directions batch contains at most 20 legs and uses one HTTP budget without an application retry. Agent continues to receive the existing `HUB_TIMEOUT_SECONDS`; changing it does not change User's ordinary calls.

The candidate co-host User/Agent/Hub/YOLO/Admin services drop every capability and set `no-new-privileges:true`. Service listeners use 8080/8000; data volumes and root filesystem mutability are unchanged. Existing central Admin API/migration already use these limits. Admin Web receives `no-new-privileges:true`, but its current root nginx master still changes worker UID/GID and cache ownership, so dropping every capability is deferred until a nonroot web image is validated. This exception does not apply to the five app processes. Apply only with the paired tested images; compare effective/permitted/bounding capabilities and NoNewPrivs on the deployed processes, then test full application readiness. Local binary/import/listener validation is not full GCP runtime acceptance.
